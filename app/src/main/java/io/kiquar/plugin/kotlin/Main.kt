...
class Main(context: ExtensionContext) : ExtensionAPI(context) {
    ...
    override fun onExtensionLoaded() {
        val icon = Icon.ExternalResourceIcon(R.drawable.kotlin, resources);
    ...
    private fun acquireLspInstallScript(): File {
        val assetStream = context.assets.open("kotlin-installer.sh")
        val assetContent = assetStream.bufferedReader().use { it.readText() }
        val scriptFile = getTempDir().child("kotlin-installer.sh").also {
